...
JuceSynthFrameworkAudioProcessor::JuceSynthFrameworkAudioProcessor()
    ...
    , tree(*this, nullptr, "PARAMETERS",
        {   std::make_unique<AudioParameterFloat>("attack", "Attack", NormalisableRange<float>(0.1f, 5000.0f), 0.1f),
            ...
            std::make_unique<AudioParameterFloat>("release", "Release", NormalisableRange<float>(0.1f, 50000.0f), 50000.0f),
            ...
            std::make_unique<AudioParameterFloat>("filterType", "FilterType", NormalisableRange<float>(0.0f, 2.0f), 0.0f),
              std::make_unique<AudioParameterFloat>("filterPole", "FilterPole", NormalisableRange<float>(0.0f, 1.0f), 1.0f),
            ...
            std::make_unique<AudioParameterFloat>("blend1", "Osc1Blend", NormalisableRange<float>(0.0f, 1.0f), 1.0f),
            ...
            std::make_unique<AudioParameterFloat>("fattack", "fAttack", NormalisableRange<float>(0.1f, 5000.0f), 0.1f),
            std::make_unique<AudioParameterFloat>("fdecay", "fDecay", NormalisableRange<float>(1.0f, 2000.0f), 1.0f),
            std::make_unique<AudioParameterFloat>("fsustain", "fSustain", NormalisableRange<float>(0.0f, 1.0f), 0.8f),
            std::make_unique<AudioParameterFloat>("frelease", "fRelease", NormalisableRange<float>(0.1f, 50000.0f), 50000.0f),
            
            //std::make_unique<AudioParameterFloat>("filter2Type", "Filter2Type", NormalisableRange<float>(0.0f, 2.0f), 0.0f),
            //std::make_unique<AudioParameterFloat>("filter2Cutoff", "Filter2Cutoff", NormalisableRange<float>(20.0f, 10000.0f), 400.0f),
            //std::make_unique<AudioParameterFloat>("filter2Res", "Filter2Res", NormalisableRange<float>(1.0f, 5.0f), 1.0f),
        })
#endif
{
    mySynth.clearVoices();
...
void JuceSynthFrameworkAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    stateVariableFilter1.reset();
    stateVariableFilter1.prepare(spec);
    updateFilter();
...
void JuceSynthFrameworkAudioProcessor::updateFilter()
{
    ...
    if (menuChoice == 0)
    {
        stateVariableFilter1.state->type = dsp::StateVariableFilter::Parameters<float>::Type::lowPass;
        stateVariableFilter1.state->setCutOffFrequency(lastSampleRate, freq, res);
    }
    
    if (menuChoice == 1)
    {
        stateVariableFilter1.state->type = dsp::StateVariableFilter::Parameters<float>::Type::highPass;
        stateVariableFilter1.state->setCutOffFrequency(lastSampleRate, freq, res);
    }
    
    if (menuChoice == 2)
    {
        stateVariableFilter1.state->type = dsp::StateVariableFilter::Parameters<float>::Type::bandPass;
        stateVariableFilter1.state->setCutOffFrequency(lastSampleRate, freq, res);
    }
}

void JuceSynthFrameworkAudioProcessor::updateFilter2()
{
    int menu2Choice = *tree.getRawParameterValue("filterType");
    int freq2 = *tree.getRawParameterValue("filterCutoff");
    int res2 = *tree.getRawParameterValue("filterRes");
    
    if (menu2Choice == 0)
    {
        stateVariableFilter2.state->type = dsp::StateVariableFilter::Parameters<float>::Type::lowPass;
        stateVariableFilter2.state->setCutOffFrequency(lastSampleRate, freq2, res2);
    }
...
void JuceSynthFrameworkAudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int i = 0; i < mySynth.getNumVoices(); i++)
    {
        ...
        if ((myVoice = dynamic_cast<SynthVoice*>(mySynth.getVoice(i))))
        {
            ...
            myVoice->getFilter2Params(tree.getRawParameterValue("filterType"),
                                     tree.getRawParameterValue("filterCutoff"),
                                     tree.getRawParameterValue("filterRes"));
            
            myVoice->getFilterEnvelopeParams(tree.getRawParameterValue("fattack"),
                                       tree.getRawParameterValue("fdecay"),
                                       tree.getRawParameterValue("fsustain"),
                                       tree.getRawParameterValue("frelease"));
    ...
    stateVariableFilter1.process(dsp::ProcessContextReplacing<float>(block));
