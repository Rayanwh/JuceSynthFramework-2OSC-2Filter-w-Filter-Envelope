...
JuceSynthFrameworkAudioProcessorEditor::JuceSynthFrameworkAudioProcessorEditor (JuceSynthFrameworkAudioProcessor& p)
    : AudioProcessorEditor (&p), processor (p), oscGui(p), osc2Gui(p), envGui(p), filenvGui(p), filterGui(p), frontGui(p)
{
    setSize (1200, 200);
...
void JuceSynthFrameworkAudioProcessorEditor::resized()
{
    ...
    filenvGui.setBounds(area.removeFromLeft(componentWidth).removeFromTop(componentHeight));
